   ...
   public boolean updateItem(Item item, int sellerId) {
    ...
    public boolean deleteItem(int itemId, int sellerId) {
        ...
        try {
            ...
            // 1. Kiểm tra trạng thái và lấy auction_id
            String sqlCheck = "SELECT auction_id, status FROM AUCTIONS WHERE item_id = ?";
            ...
            int auctionId = -1;
            if (rs.next()) {
                if ("RUNNING".equalsIgnoreCase(rs.getString("status"))) {
