...
public void updateAuctionStatus(int auctionId, String status) {
   ...
   public int placeSingleBid(double amount, int userId, int auctionId) {
        Connection conn = null;
        try {
            // Dùng đúng config.DatabaseConnection của bạn
            conn = config.DatabaseConnection.getInstance().getConnection();
            conn.setAutoCommit(false); // Bắt đầu Transaction bằng Java (Thay thế Stored Procedure)

            // 1. Kiểm tra an toàn: Cột balance có tồn tại không?
            try (Statement stmt = conn.createStatement()) {
                stmt.executeQuery("SELECT balance FROM USERS LIMIT 1");
            } catch (SQLException e) {
                System.err.println("LỖI NGHIÊM TRỌNG: Bảng USERS thiếu cột 'balance'!");
                System.err.println("Hãy chạy lệnh SQL: ALTER TABLE USERS ADD COLUMN balance DECIMAL(15,2) DEFAULT 0.0;");
                return -2;
            }

            // 2. Lấy thông tin phiên đấu giá (Chống đọc ghi đồng thời bằng FOR UPDATE)
            double startingPrice = 0, currentMax = 0;
            String status = "";
            boolean hasCurrentMax = false;

            String selAuction = "SELECT starting_price, current_max_price, status FROM AUCTIONS WHERE auction_id = ? FOR UPDATE";
            try (PreparedStatement ps = conn.prepareStatement(selAuction)) {
                ps.setInt(1, auctionId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) {
                        startingPrice = rs.getDouble("starting_price");
                        double cmp = rs.getDouble("current_max_price");
                        if (!rs.wasNull()) {
                            currentMax = cmp;
                            hasCurrentMax = true;
                        }
                        ...
                        conn.rollback(); return -1; // Lỗi: Không tìm thấy phiên
                    }
                }
            }

            if (!"RUNNING".equals(status)) {
                conn.rollback(); return -1; // Lỗi: Phiên đã đóng
            }

            // 3. Lấy số dư hiện tại của người dùng
            double balance = 0;
            String selBalance = "SELECT balance FROM USERS WHERE user_id = ? FOR UPDATE";
            try (PreparedStatement ps = conn.prepareStatement(selBalance)) {
                ps.setInt(1, userId);
                try (ResultSet rs = ps.executeQuery()) {
                    if (rs.next()) balance = rs.getDouble("balance");
                    else { conn.rollback(); return -2; }
                }
            }

            if (balance < amount) {
                conn.rollback(); return -3; // Lỗi: Không đủ tiền
            }

            // 4. Kiểm tra logic giá và Trừ tiền
            double targetPrice = hasCurrentMax ? currentMax : startingPrice;
            if (amount > targetPrice || (!hasCurrentMax && amount == startingPrice)) {
                
                // Lấy ID người giữ giá cao nhất cũ để hoàn tiền
                int prevBidder = -1;
                String selTop = "SELECT bidder_id FROM BID_TRANSACTIONS WHERE auction_id = ? ORDER BY bid_amount DESC, bid_time ASC LIMIT 1";
                try (PreparedStatement ps = conn.prepareStatement(selTop)) {
                    ps.setInt(1, auctionId);
                    try (ResultSet rs = ps.executeQuery()) {
                        if (rs.next()) prevBidder = rs.getInt("bidder_id");
                    }
                }

                // Trừ tiền người mua mới
                String updDec = "UPDATE USERS SET balance = balance - ? WHERE user_id = ?";
                try (PreparedStatement ps = conn.prepareStatement(updDec)) {
                    ps.setDouble(1, amount);
                    ps.setInt(2, userId);
                    ps.executeUpdate();
                }

                // Hoàn tiền cho người bị vượt giá (Nếu có)
                if (prevBidder != -1 && hasCurrentMax) {
                    String updInc = "UPDATE USERS SET balance = balance + ? WHERE user_id = ?";
                    try (PreparedStatement ps = conn.prepareStatement(updInc)) {
                        ps.setDouble(1, currentMax);
                ...
                String insBid = "INSERT INTO BID_TRANSACTIONS (auction_id, bidder_id, bid_amount) VALUES (?, ?, ?)";
                try (PreparedStatement ps = conn.prepareStatement(insBid)) {
                    ps.setInt(1, auctionId);
                    ps.setInt(2, userId);
                ...
                // Cập nhật giá mới cho phiên đấu giá
                String updAuc = "UPDATE AUCTIONS SET current_max_price = ? WHERE auction_id = ?";
                try (PreparedStatement ps = conn.prepareStatement(updAuc)) {
                    ps.setDouble(1, amount);
                    ps.setInt(2, auctionId);
                    ps.executeUpdate();
                }

                conn.commit(); // XÁC NHẬN TOÀN BỘ GIAO DỊCH THÀNH CÔNG
                return 1;
            } else {
                conn.rollback(); return 0; // Giá đặt quá thấp
            }

        } catch (SQLException e) {
            if (conn != null) try { conn.rollback(); } catch (SQLException ex) {}
            System.err.println("Lỗi Transaction JDBC: " + e.getMessage());
            e.printStackTrace();
            return -2;
        } finally {
            if (conn != null) {
